...
const canvas = new Stage("fireworksCanvas");
...
const SELECT_QUALITY = QUALITY_HIGH;
const SELECT_SKY_LIGHT = SKY_LIGHT_NORMAL;
const SELECT_SHELL_SIZE = 6;
...
// Sequences
// -----------
function seqRandomPositionFire() {
...
function handleResize() {
  ...
  canvas.resize(containerW, containerH);
  // Account for scale
...
function updateGlobals(timeStep, lag) {
  ...
  if (autoLaunchTime <= 0) {
    autoLaunchTime = seqRandomPositionFire() * 1.25;
  }
}
...
function render(speed) {
  ...
  const scaleFactor = getDefaultScaleFactor();
  canvasCtx.scale(dpr * scaleFactor, dpr * scaleFactor);

  canvasCtx.globalCompositeOperation = "destination-out";
  canvasCtx.fillStyle = `rgba(0, 0, 0, ${0.175 * speed
    })`;
  ...
  while (BurstFlash.active.length) {
    ...
    const burstGradient = canvasCtx.createRadialGradient(
      bf.x,
      bf.y,
      0,
      bf.x,
    ...
    canvasCtx.fillStyle = burstGradient;
    canvasCtx.fillRect(
      bf.x - bf.radius,
      bf.y - bf.radius,
      bf.radius * 2,
  ...
  // Draw stars
  canvasCtx.lineWidth = Star.drawWidth;
  canvasCtx.lineCap = isLowQuality ? "square" : "round";
  COLOR_CODES.forEach((color) => {
    ...
    stars.forEach((star) => {
      if (star.visible) {
        canvasCtx.moveTo(star.x, star.y);
        canvasCtx.lineTo(star.prevX, star.prevY);
      }
    });
    canvasCtx.stroke();
  });
  ...
  // Draw sparks
  canvasCtx.lineWidth = Spark.drawWidth;
  ...
  COLOR_CODES.forEach((color) => {
    ...
    sparks.forEach((spark) => {
      canvasCtx.moveTo(spark.x, spark.y);
      canvasCtx.lineTo(spark.prevX, spark.prevY);
    });
    canvasCtx.stroke();
  });
...
canvas.addEventListener("ticker", update);
