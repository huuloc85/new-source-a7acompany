drop procedure if exists trg_insert_wc_clean_men_to_schedule_details;
CREATE PROCEDURE trg_insert_wc_clean_men_to_schedule_details(IN p_employee_id BIGINT, IN p_celender_id BIGINT)
BEGIN
    -- Use a single query with UNION to handle all days at once without dynamic SQL
    INSERT INTO schedule_details (employee_id, schedule_id, date, is_wc_clean_men, created_at, updated_at)
    SELECT 
        detail.employee_id,
        detail.celender_id,
        ...
        END as is_wc_clean_men,
        NOW(),
        NOW()
    FROM celender_detail_wc_clean_men detail
    ...
    ON DUPLICATE KEY
        UPDATE is_wc_clean_men = VALUES(is_wc_clean_men),
               updated_at = NOW();
END;
