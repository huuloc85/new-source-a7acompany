...
class ScheduleCategoryController extends BaseController
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            return response()->json(['success' => false, 'message' => 'Lỗi lấy danh sách danh mục: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function show($id)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::error(basename(__FILE__) . ' - ' . __FUNCTION__ . ' - Error: ' . $e->getMessage());
            return response()->json(['success' => false, 'message' => 'Lỗi lấy chi tiết danh mục: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::error(basename(__FILE__) . ' - ' . __FUNCTION__ . ' - Error: ' . $e->getMessage());
            return response()->json(['success' => false, 'message' => 'Lỗi tạo danh mục: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function delete($id)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::error(basename(__FILE__) . ' - ' . __FUNCTION__ . ' - Error: ' . $e->getMessage());
            return response()->json(['success' => false, 'message' => 'Lỗi xoá danh mục: ' . $e->getMessage()], 500);
        }
    }
}
