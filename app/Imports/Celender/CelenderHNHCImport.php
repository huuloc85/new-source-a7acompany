...
class CelenderHNHCImport implements SkipsEmptyRows, SkipsOnFailure, ToArray, WithStartRow, WithValidation
{
    ...
    {
        try {
            $existing = $this->getExistingEmployeeSet(CelenderDetailHNHC::class, $this->celenderId);
            $now = now();
            $insertRows = [];
            ...
            foreach ($rows as $row) {
                ...
                $insertRows[] = array_merge([
                    ...
                    'created_at' => $now,
                    'updated_at' => $now,
                ], $this->extractDays($row, 4, 31));

                $existing[$employeeId] = true;
            }

            $this->batchInsert(CelenderDetailHNHC::class, $insertRows);
        } catch (\Throwable $e) {
            LogHelper::saveLog('Import-Celender-HN', $e->getMessage(), $e->getLine());
            Log::error('Import Celender HN error: '.$e->getMessage().' line '.$e->getLine());
            throw $e;
        }
    }
    ...
    public function onFailure(Failure ...$failures)
    {
        foreach ($failures as $failure) {
            LogHelper::saveLog('Import-Celender-HN', $failure->errors()[0], $failure->row());
        }
    }
}
