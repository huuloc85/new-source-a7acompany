    ...
    WithStartRow
{
        ...
        try {
            foreach ($rows as $row) {
                if ($row[1] != null && $row[1] != '') {
                    ...
                    if ($employee != null) {
                        $checkCelenderDetailWCClean = CelenderDetailWCClean::where('celender_id', $this->celenderId)->where('employee_id', $employee->id)->first();
                        if ($checkCelenderDetailWCClean == null) {
                            CelenderDetailWCClean::create([
    ...
    public function onFailure(Failure ...$failures)
    {
        foreach ($failures as $key => $failure) {
            LogHelper::saveLog('Import-Celender-WC-Clean', $failure->errors()[0], $failure->row());
        }
    }
}
