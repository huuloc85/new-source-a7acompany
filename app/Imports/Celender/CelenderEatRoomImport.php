...
class CelenderEatRoomImport implements SkipsEmptyRows, SkipsOnFailure, ToArray, WithStartRow, WithValidation
{
    ...
    {
        try {
            $existing = $this->getExistingEmployeeSet(CelenderDetailEatroom::class, $this->celenderId);
            $now = now();
            $insertRows = [];
            ...
            $this->batchInsert(CelenderDetailEatroom::class, $insertRows);
        } catch (\Throwable $e) {
            LogHelper::saveLog('Import-Celender-EatRoom', $e->getMessage(), $e->getLine());
            Log::error('Import Celender EatRoom error: '.$e->getMessage().' line '.$e->getLine());
            throw $e;
        }
    }
    ...
    public function onFailure(Failure ...$failures)
    {
        foreach ($failures as $failure) {
            LogHelper::saveLog('Import-Celender-EatRoom', $failure->errors()[0], $failure->row());
        }
    }
}
