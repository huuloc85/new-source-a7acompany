...
class CelenderWCCleanWomenImport implements HasReferencesToOtherSheets, SkipsEmptyRows, SkipsOnFailure, ToArray, WithStartRow, WithValidation
{
    public $celenderId;
    ...
    {
        return 'Trực WC Nữ'; // Đặt tên sheet ở đây
    }
        ...
        try {
            foreach ($rows as $row) {
                if ($row[1] != null && $row[1] != '') {
                    ...
                    if ($employee != null) {
                        $checkCelenderDetailWCCleanWomen = CelenderDetailWCCleanWomen::where('celender_id', $this->celenderId)->where('employee_id', $employee->id)->first();
                        if ($checkCelenderDetailWCCleanWomen == null) {
                            CelenderDetailWCCleanWomen::create([
