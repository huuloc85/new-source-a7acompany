...
class CelenderWCCleanWomenImport implements SkipsEmptyRows, SkipsOnFailure, ToArray, WithStartRow, WithValidation
{
    ...
    {
        try {
            $existing = $this->getExistingEmployeeSet(CelenderDetailWCCleanWomen::class, $this->celenderId);
            $now = now();
            $insertRows = [];
            ...
            $this->batchInsert(CelenderDetailWCCleanWomen::class, $insertRows);
        } catch (\Throwable $e) {
            LogHelper::saveLog('Import-Celender-WC-Clean-Women', $e->getMessage(), $e->getLine());
            Log::error('Import Celender WC Clean Women error: '.$e->getMessage().' line '.$e->getLine());
            throw $e;
        }
    }
    ...
    public function onFailure(Failure ...$failures)
    {
        foreach ($failures as $failure) {
            LogHelper::saveLog('Import-Celender-WC-Clean-Women', $failure->errors()[0], $failure->row());
        }
    }
}
