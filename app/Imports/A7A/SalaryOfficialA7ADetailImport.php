...
class SalaryOfficialA7ADetailImport implements HasReferencesToOtherSheets, SkipsEmptyRows, SkipsOnFailure, ToArray, WithCalculatedFormulas, WithChunkReading, WithEvents, WithStartRow, WithValidation
{
                ...
                if ($row[1] != null && $row[1] != '') {
                    ...
                    if ($employee != null && $this->salaryManagerId != null) {
                        ...
                        if ($salaryManager) {
                            ...
                            $salaryManager->subtract_of_violations = (is_numeric($row[71] ?? null) ? (float) $row[71] : null);                                 // Trừ vi phạm
                            $salaryManager->subtract_of_violations_notice = (is_numeric($row[72] ?? null) ? (float) $row[72] : null);                          // Trừ vi phạm Ghi Chú
    ...
    public function customValidationMessages()
    {
        ...
        $role = [
            ...
            'Trừ tiền số lỗi nhẹ Ghi Chú', 'trừ KPI', 'thực lãnh',
