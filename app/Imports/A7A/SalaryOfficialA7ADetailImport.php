...
class SalaryOfficialA7ADetailImport implements HasReferencesToOtherSheets, SkipsEmptyRows, SkipsOnFailure, ToArray, WithCalculatedFormulas, WithChunkReading, WithEvents, WithStartRow, WithValidation
{
    ...
    public function __construct($salaryManagerId)
    {
        ...
        $this->roleIgnore = [5, 8, 10, 12, 14, 17, 20, 23, 26, 29, 32, 35, 39, 43, 45, 47, 49, 51, 53, 55, 58, 60, 62, 64, 66, 68, 70, 72, 74];
    }
    ...
    private function fillDetailAttributes(SalaryOfficialA7A $salaryManager, array $row): void
    {
    ...
    private function detailFields(): array
    {
        return [
            ...
            ['attribute' => 'subtract_of_violations', 'header' => 'Trừ vi phạm', 'fallback' => 71, 'type' => 'numeric'],
            ['attribute' => 'subtract_of_violations_notice', 'noticeAfter' => 'Trừ vi phạm', 'fallback' => 72, 'type' => 'string'],
    ...
    public function onFailure(Failure ...$failures)
    {
        foreach ($failures as $key => $failure) {
            LogHelper::saveLog('Import-A7A-Chi tiết', $this->formatImportFailure($this->sheet(), $failure), $failure->row());
        }
    }
